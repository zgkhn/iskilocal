...
import { applyScaleMath, formatNumber } from '@/lib/scaling';
...
export default function TagScalesPage() {
    ...
    const [form, setForm] = useState({
        ...
        offset_value: 0, decimal_precision: 2,
    ...
    const handleSave = async () => {
        const body = {
            ...
            max_digits: form.max_digits ? parseInt(form.max_digits) : null,
        };
    ...
    const preview = (raw = 8754) => {
        const scale = {
            multiply_factor: parseFloat(form.multiply_factor) || 1,
            divide_factor: parseFloat(form.divide_factor) || 1,
            offset_value: parseFloat(form.offset_value) || 0,
            decimal_precision: form.decimal_precision,
            decimal_separator: form.decimal_separator,
            thousand_separator: form.thousand_separator,
            max_digits: form.max_digits ? parseInt(form.max_digits) : null,
            unit: form.unit
        };
        const scaled = applyScaleMath(raw, scale);
        const formatted = formatNumber(scaled, scale);
        return formatted + (form.unit ? ` ${form.unit}` : '');
    };
        ...
        <div>
            ...
            <div className="data-table-wrapper">
                <table className="data-table">
                    ...
                    <tbody>
                        {scales.map(s => {
                            let val = applyScaleMath(8754, s);
                            let demo = formatNumber(val, s);
                            return (
                                <tr key={s.id}>
                                    ...
                                    <td>
                                        <button className="btn btn-sm btn-secondary" style={{ marginRight: 4 }}
                                            onClick={() => {
                                                setForm({
                                                    tag_id: s.tag_id,
                                                    multiply_factor: s.multiply_factor,
                                                    divide_factor: s.divide_factor,
                                                    ...
                                                    max_digits: s.max_digits || '',
                                                    unit: s.unit || ''
                                                });
                                                setSelectedPlc(''); // Reset selection or find PLC/Table
                                                setShowModal(true);
                                            }}>✏️</button>
            ...
            {showModal && (
                <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
                    <div className="modal">
                        ...
                        <div className="form-row">
                            <div className="form-group">
                                <label className="form-label">Ondalık Ayracı</label>
                                <select className="form-select" value={form.decimal_separator}
                            ...
                            <div className="form-group">
                                <label className="form-label">Binlik Ayracı</label>
                                <select className="form-select" value={form.thousand_separator}
                        ...
                        <div className="form-row">
                            ...
                            <div className="form-group">
                                <label className="form-label">Maks. Karakter</label>
                                <input type="number" className="form-input" value={form.max_digits} placeholder="Örn: 8"
                                    onChange={e => setForm(f => ({ ...f, max_digits: e.target.value }))} />
                            </div>
                        </div>
                        <div className="form-group">
